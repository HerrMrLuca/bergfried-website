...
final class Login
{
    ...
    public function isValid(): void
    {
        if (Utilities::isEmptyString($_POST['password'])) {
            $this->messages[] = "Enter a password";
        }
        if (Utilities::isEmptyString($_POST['username'])) {
            $this->messages[] = "Enter a email";
        }
        ...
        $this->twigParams['username'] = $_POST['username'];
...
SQL;
        ...
        if (count($rows) === 1 && strcmp($_POST['password'], $rows[0]->password) < 2) {
...
SQL;
        $params = [':password' => $password, ':user_id' => $_SESSION['user_id']];
        if ($this->dbh) {
            $this->stmt = $this->dbh->prepare($query);
            $this->stmt->execute($params);
        }
